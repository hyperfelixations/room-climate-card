...
|---|---:|---|---|---|
| `dom/` | 32 | `render/composition`, `render/primitives`, `views/` | The complete rendered markup for 32 configurations. The broadest safety net the suite has: any change in structure, class, attribute or order shows up here first. | The markup change is the intended result of the commit, and the diff has been read line by line. |
| `model/` | 32 | `application/model`, `presentation/view-model` | The same 32 configurations as data, through the frozen flat DTO. A **Phase 0 oracle**: recorded against the ORIGINAL monolithic card, and therefore the only independent evidence that the extracted pipeline still computes what it always computed. | Almost never. A change here means the extracted pipeline now computes something different from the card these were taken from, which needs a reason in the changelog. |
