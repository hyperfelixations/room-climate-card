...
the 79 files under `test/baseline/` has an owner, a purpose, and a rule for when it may be
...
|---|---:|---|---|---|
| `dom/` | 34 | `render/composition`, `render/primitives`, `views/` | The complete rendered markup for 34 configurations. The broadest safety net the suite has: any change in structure, class, attribute or order shows up here first. | The markup change is the intended result of the commit, and the diff has been read line by line. |
| `model/` | 34 | `application/model`, `presentation/view-model` | The same 34 configurations as data, through the frozen flat DTO. A **Phase 0 oracle**: recorded against the ORIGINAL monolithic card, and therefore the only independent evidence that the extracted pipeline still computes what it always computed. | Almost never. A change here means the extracted pipeline now computes something different from the card these were taken from, which needs a reason in the changelog. |
...
| `diagnostics/` | 3 | `config/`, `application/model`, `presentation/view-model` | Every refusal, warning and hint the card emits, verbatim and in English: the refusals `setConfig()` throws, the warnings of a configuration, and the warnings and hints its sensors cause. These are a user-facing contract: the text is what somebody reads at two in the morning. | The wording is being improved on purpose. A message that changes by accident is a regression. |
