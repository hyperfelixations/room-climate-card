...
test("XSS payload in title/avg_label config overrides produces no extra DOM nodes", () => {
  const hass = mkHass({ "sensor.avg": mkState("sensor.avg", 22, { device_class: "temperature" }) });
  const el = env.createCard({ entity: "sensor.avg", title: XSS_PAYLOAD, avg_label: XSS_SCRIPT }, hass);
  assert.equal(countInjectedNodes(el.shadowRoot), 0);
  env.cleanup(el);
});
