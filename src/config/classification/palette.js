...
//   palette: {ramp, invalid}  a palette written out in YAML
//
...
export function normalizePalette(value, { paletteForName, paletteNames, assertPalette }) {
  ...
  if (typeof value === "string") {
    ...
    if (!palette) {
      ...
      pathError("palette", `"${value}" is not a known palette — available: ${known}, or write one out as {ramp, invalid}`);
    }
    return palette;
  }

  if (!isPlainObject(value)) pathError("palette", "must be a palette name or an object with ramp and invalid");
  assertAllowedKeys(value, new Set(["ramp", "invalid"]), "palette");
  ...
  return assertPalette({ id: "custom", ramp: value.ramp, invalid: value.invalid }, "palette");
}
